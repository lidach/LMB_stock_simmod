						...
						DD_flag = TRUE)
{
    ...
    Wt <- alfa*TL_global^bet # weight at age
		...
		for(y in 2:years){
			...
			if(DD_flag) avg_size[y,] <- (colSums(nage[y,,]*TL_bar[y,,]*Vulcat) + colSums(nage_hat[y,,]*TL_bar[y,,]*Vulcat) + colSums(nage_st[y,,]*TL_bar[y,,]*Vulcat)) /
				          (colSums(nage[y,,]*Vulcat) + colSums(nage_hat[y,,]*Vulcat) + colSums(nage_st[y,,]*Vulcat))
		...
		res <- list(et = et, hr = hr, WSB = WSB, st = st, R = R,
